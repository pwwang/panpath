...
class PanPathMeta(type):
    """Metaclass for PanPath that dispatches based on URI scheme and mode."""
...
class PanPath(metaclass=PanPathMeta):
...
class AsyncPanPathMeta(type):
    """Metaclass for AsyncPanPath that always returns async paths."""
...
class AsyncPanPath(metaclass=AsyncPanPathMeta):
