"""OmegaPath - Universal sync/async local/cloud path library.

Examples:
    >>> from omegapath import OmegaPath, AsyncOmegaPath
    >>>
    >>> # Sync local path
    >>> path = OmegaPath("/path/to/file.txt")
    ...
    >>>
    >>> # Async S3 path
    >>> async_path = OmegaPath("s3://bucket/key.txt", mode="async")
    >>> content = await async_path.read_text()
    >>>
    >>> # AsyncOmegaPath (always async)
    >>> async_path = AsyncOmegaPath("s3://bucket/key.txt")
    >>> content = await async_path.read_text()
"""

from omegapath.router import AsyncOmegaPath, OmegaPath
...
from omegapath.registry import register_path_class

# Register S3
try:
    from omegapath.s3_sync import S3Path
    from omegapath.s3_async import AsyncS3Path
...
try:
    from omegapath.gs_sync import GSPath
    from omegapath.gs_async import AsyncGSPath
...
try:
    from omegapath.azure_sync import AzureBlobPath
    from omegapath.azure_async import AsyncAzureBlobPath
